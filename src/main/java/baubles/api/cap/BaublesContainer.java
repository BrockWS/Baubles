...
public class BaublesContainer extends ItemStackHandler implements IBaublesItemHandler {
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack, EntityLivingBase player) {
        if (stack == null || stack.isEmpty())
            return false;
        IBauble bauble = stack.getCapability(BaublesCapabilities.CAPABILITY_ITEM_BAUBLE).orElse(s -> null);
        return bauble != null && bauble.canEquip(stack, player) && bauble.getBaubleType(stack).hasSlot(slot);
    }
